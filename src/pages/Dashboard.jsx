...
import { Users, DoorOpen, Wrench, CalendarOff, Package, ShieldAlert, Building2, ClipboardCheck } from 'lucide-react';
  ...
  useEffect(() => {
    async function load() {
      ...
      const blockDist = blocks.map(b => {
        ...
        return { name: b.block_name, occupancy: bRooms.reduce((a, r) => a + (r.current_occupancy || 0), 0), capacity: bRooms.reduce((a, r) => a + (r.capacity || 0), 0) };
      });

      const maintByCategory = ['Electrical', 'Plumbing', 'Furniture', 'Internet', 'Cleaning', 'Others'].map(cat => ({
        name: cat, count: maintenance.filter(m => m.category === cat).length
      })).filter(c => c.count > 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {stats.leaveByStatus.length > 0 && (
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={stats.leaveByStatus} cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                  {stats.leaveByStatus.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
        ...
        {stats.blockDist.length === 0 && stats.maintByCategory.length === 0 && stats.leaveByStatus.length === 0 && (
          <div className="bg-card border border-border rounded-xl p-5 col-span-full flex items-center justify-center h-48">
            <p className="text-sm text-muted-foreground">Add data to see charts here.</p>
          </div>
        )}
      </div>
    </div>
