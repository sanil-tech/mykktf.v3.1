...
const REVIEWER_ROLES = ['warden', 'admin', 'staff'];
...
export default function Leave() {
  ...
  const [form, setForm] = useState({ leave_type: 'Weekend', destination: '', reason: '', departure_date: '', departure_time: '', return_date: '', return_time: '' });
  ...
  async function init() {
    ...
    if (isRev) {
      ...
      leaveList = student
        ? await base44.entities.LeaveApplication.filter({ student_id: student.id })
        : [];
    }
    setApps(leaveList);
    setLoading(false);
  ...
  async function updateStatus(id, status) {
    await base44.entities.LeaveApplication.update(id, { status, approved_by: currentUser?.full_name || currentUser?.email });
  ...
  return (
    <div>
      <PageHeader
        ...
        description={isReviewer ? "Review student leave requests" : "Your leave applications"}
        actions={canApply && (
          <Button size="sm" onClick={() => { setForm({ leave_type: 'Weekend', destination: '', reason: '', departure_date: '', departure_time: '', return_date: '', return_time: '' }); setDialogOpen(true); }}>
