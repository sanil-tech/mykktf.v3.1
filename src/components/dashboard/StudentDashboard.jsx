...
function QuickAction({ to, icon: Icon, label, color, description, isPriority }) {
  return (
    <Link 
      to={to} 
    ...
    <div className="space-y-6 bg-[#F8FAFC] p-1 rounded-2xl">
      ...
      {(() => {
        ...
        const label = isOverdue
          ? '⚠️ Tempoh 48 Jam Telah Tamat — Peringatan Demerit & Liabiliti Bilik'
          : isWarning
          ? '⏰ Kurang 24 Jam — Hantar Laporan Pemeriksaan Bilik'
          : '📋 Pemeriksaan Bilik Wajib (48 Jam Pertama)';
        const desc = isOverdue
          ? 'Anda telah melebihi tempoh 48 jam. Mengikut peraturan KKTF, kegagalan menghantar laporan boleh dikenakan demerit disiplin dan sebarang kerosakan sedia ada akan dianggap di bawah liabiliti penghuni.'
          : isWarning
          ? 'Sila semak 8 komponen inventori bilik dan hantar laporan segera sebelum tempoh 48 jam tamat untuk mengelakkan penalti merit.'
          : 'Sila periksa 8 inventori bilik dan hantar laporan dalam tempoh 48 jam selepas menerima kunci bilik kolej untuk melindungi rekod anda.';
          ...
          <div className={`border rounded-2xl p-4 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-3 ${bannerCls}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                ...
                {isOverdue && (
                  <div className="flex items-center gap-1.5 text-[11px] font-semibold text-rose-800 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200 w-fit mt-1">
                    <ShieldAlert className="w-3.5 h-3.5 text-rose-600 shrink-0" />
                    <span>Status: Rekod ditandakan untuk semakan Felo Blok & risiko demerit kolej.</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 mb-3 pl-1">
          ...
          {!hasInspectionDone && (
            <span className="text-[11px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2.5 py-0.5 rounded-full flex items-center gap-1.5 w-fit">
              <CheckSquare className="w-3 h-3 text-amber-600" /> Peringatan: Sila lengkapkan Pemeriksaan Bilik (48 Jam)
            </span>
          )}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          ...
          <QuickAction 
            ...
            description="Semak status inspeksi bilik anda" 
            color="bg-green-600" 
